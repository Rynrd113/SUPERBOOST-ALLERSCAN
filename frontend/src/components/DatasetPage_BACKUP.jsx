...
// Reusable Components - BreakdownCard untuk statistik breakdown
...
function DatasetPage({ onNavigate }) {
  ...
  const loadComprehensiveStatistics = useCallback(async () => {
    try {
      ...
      if (statsResponse.data?.success && statsResponse.data?.data) {
        const comprehensive = statsResponse.data.data
        console.log('✅ Comprehensive statistics loaded:', comprehensive)
        
        setStats(prevStats => ({
          ...prevStats,
          // Update with actual backend data
          total: comprehensive.overview?.total_predictions || prevStats.total,
          averageConfidence: comprehensive.performance_metrics?.confidence_distribution || prevStats.averageConfidence,
          recentActivity: comprehensive.overview?.total_predictions || prevStats.recentActivity,
          
          // Use actual allergen data from backend
          allergenBreakdown: comprehensive.chart_data?.allergens_distribution?.reduce((acc, item) => {
            acc[item.name] = item.count
            return acc
          }, {}) || prevStats.allergenBreakdown,
          
          detectionRate: comprehensive.overview?.detection_rate || prevStats.detectionRate,
          
          // Risk level breakdown (actual data)
          riskBreakdown: comprehensive.chart_data?.risk_distribution?.reduce((acc, item) => {
            acc[item.level] = item.count
            return acc
          }, {}) || prevStats.riskBreakdown
        }))
      }
    } catch (error) {
      console.warn('⚠️ Failed to load comprehensive statistics:', error.message)
      // Don't throw - fall back to basic statistics
    }
  }, [])
  ...
  const loadData = useCallback(async (page = 1, pageSize = 20) => {
    try {
      ...
      if (response.data) {
        let predictions = []
        let paginationData = {}
        // let statsData = null  // Unused variable removed
        
        // Handle different response formats
        if (response.data.data) {
          predictions = response.data.data.predictions || []
          paginationData = response.data.data.pagination || {}
          // statsData = response.data.data.statistics || null  // Unused variable removed
        } else {
          predictions = response.data.predictions || []
          paginationData = response.data.pagination || {}
          // statsData = response.data.statistics || null  // Unused variable removed
        }
